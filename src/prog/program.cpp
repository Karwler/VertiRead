...
void Program::eventUser(const SDL_UserEvent& user) {
	...
	case UserCode::readerProgress:
		World::scene()->setPopup(ProgState::createPopupMessage("Loading " + static_cast<PictureLoader*>(user.data1)->progVal + '/' + static_cast<PictureLoader*>(user.data1)->progLim, &Program::eventReaderLoadingCancelled, "Cancel", Alignment::center));
		break;
	...
	case UserCode::moveFinished:
		...
		break;
	default:
		throw std::runtime_error("Invalid user event code: " + to_string(user.code));
	}
}
...
void Program::eventStartLoadingReader(const string& first, bool fwd) {
	World::scene()->setPopup(ProgState::createPopupMessage("Loading...", &Program::eventReaderLoadingCancelled, "Cancel", Alignment::center));
	threadRunning = true;
...
void Program::eventMoveComics(Button*) {
	World::scene()->setPopup(ProgState::createPopupMessage("Moving...", &Program::eventReaderLoadingCancelled, "Cancel", Alignment::center));
...
void Program::eventMoveProgress(uptrt prg, uptrt lim) {
	World::scene()->setPopup(ProgState::createPopupMessage("Moving " + to_string(prg) + '/' + to_string(lim), &Program::eventReaderLoadingCancelled, "Cancel", Alignment::center));
}
...
void Program::eventSetDeadzoneSL(Button* but) {
	...
	static_cast<ProgSettings*>(state)->deadzoneLE->setText(to_string(World::sets()->getDeadzone()));
}
